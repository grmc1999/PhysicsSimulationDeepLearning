# Code for generalization of diffussion solver for anisotropic diffusion
...
def differential(u: Field,
    ...
    if spatial(diffusivity):  # spatially-varying diffusivity
        ...
        x,y=unstack(sum(diffusivity,"KK"),"k")
        spatial_diffusion=Field(u.geometry,values=vec(x=x,y=y))
        if u.is_grid and u.is_centered:
            ...
            lap=u.with_values(sum(ddu*spatial_diffusion,"vector"))
            #lap=u.with_values(math.dot(
            #        ddu.sample(u.geometry),"vector",
            #        face_diffusivity.sample(u.geometry),"vector"))
        else:
